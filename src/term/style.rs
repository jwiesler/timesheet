use ratatui::style::{Color, Style};
use ratatui::symbols::border;
// pub const AQUA: Color = Color::Rgb(0, 0xFF, 0xFF);
// pub const LIGHT_SKY_BLUE: Color = Color::Rgb(0x87, 0xCE, 0xFA);

pub const HIGHLIGHT: Style = Style::new().bg(Color::Blue).bold();
pub const BORDER: Style = Style::new().fg(Color::Green);
pub const BORDER_INACTIVE: Style = Style::new().fg(Color::Gray);
pub const PROJECT: Style = Style::new().fg(Color::LightGreen);
pub const TIME: Style = Style::new().fg(Color::LightMagenta);
pub const DATE: Style = Style::new().fg(Color::LightYellow);
pub const BORDERS: border::Set<'static> = border::ROUNDED;

pub fn border(active: bool) -> Style {
    if active { BORDER } else { BORDER_INACTIVE }
}
